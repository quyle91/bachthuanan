...
$___->shortcode_name     = 'adminz_woocommerce_taxonomy_list';
$___->shortcode_title    = 'Product terms';
...
$___->options            = function(){
	...
	$repeater_col_spacing = "normal";
	$tax_arr = adminz_tmp( 'adminz_taxonomies_options', adminz_get_taxonomy_options( 'product' ) );
	$options = array(
		'style_options'         => array(
			'type'    => 'group',
			'heading' => __( 'Style' ),
...
$___->shortcode_callback = function ($atts, $content = null, $tag = '') {
	$default = [ 
		// Meta
		'number'              => null,
		'_id'                 => 'cats-' . rand(),
	...
	if ( $product_attributes ) {
		foreach ( $product_attributes as $category ) {
			...
			if ( $portfolio_tax = (adminz_get_settings('Flatsome')['adminz_flatsome_portfolio_product_tax'] ?? false) ) {
