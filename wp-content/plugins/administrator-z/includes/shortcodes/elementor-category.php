...
add_action( 'elementor/widgets/widgets_registered', function(){
	class ADMINZ_Category extends \Elementor\Widget_Base {
		...
		public function get_keywords() {
			return [ ADMINZ_NAME, 'taxonomy', 'category' ];
		}
		...
		function call_children_html($categoryHierarchy){
			...
			ob_start();
			foreach ($categoryHierarchy as $key=> $cat) {
				if (!empty($cat->children)) {
					?>
					<div class="elementor-toggle-item">
						<div class="elementor-tab-titlea <?php echo ($active); ?>" data-tab="<?php echo ($cat->slug) ; ?>"> <span class="elementor-toggle-icon elementor-toggle-icon-closed"> <?php echo adminz_get_icon('chevron-down',['width'=> "1em"]); ?> </span> <span class="elementor-toggle-icon elementor-toggle-icon-opened"> <?php echo adminz_get_icon('chevron-up',['width'=> "1em"]); ?> </span> <a class="term_link" href="<?php echo get_term_link($cat);?>"><?php echo ($cat->name);?></a> 
						</div>
	...
	\Elementor\Plugin::instance()->widgets_manager->register( new ADMINZ_Category() );
} );